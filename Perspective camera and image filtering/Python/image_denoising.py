...
matrix, single, vector = np.linalg.svd(g)
horizont = np.sqrt(single[0]) * matrix[:, 0]
vertical = np.sqrt(single[0]) * vector[0, :]

gflt_imns = conv1(conv1(imns, horizont, axis=0, mode='reflect'), vertical, axis=1, mode='reflect')
gflt_imng = conv1(conv1(imng, horizont, axis=0, mode='reflect'), vertical, axis=1, mode='reflect')
##--your-code-ends-here--##
...
def median_filter(img, wsize):
    ...
    for i in range(nrows):
        for j in range(ncols):
            ...
            ##--your-code-starts-here--##
            patch_to_median = img[iMin:iMax+1, jMin:jMax+1]
            output[i, j] = np.median(patch_to_median)
            ##--your-code-ends-here--##
