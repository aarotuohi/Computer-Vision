...
for i in range(npts1):
    for j in range(npts2):
        numerator = np.sum((patches1[:, :, i] - np.mean(patches1[:, :, i])) * (patches2[:, :, j] - np.mean(patches2[:, :, j])))
        denominator = np.sqrt(np.sum((patches1[:, :, i] - np.mean(patches1[:, :, i])) ** 2) * np.sum((patches2[:, :, j] - np.mean(patches2[:, :, j])) ** 2))
        distmat[i, j] = numerator / denominator


pairs = []
for k in range(npts1):
    if k == np.argmax(distmat, axis=0)[np.argmax(distmat, axis=1)[k]]:
        pairs.append(np.array([k, np.argmax(distmat, axis=1)[k], np.amax(distmat, axis=1)[k]]))
pairs = np.array(pairs)

sort_matches = np.sort(pairs[:,2], axis=0)[::-1]
